...
int current_circulation_fan3_speed = 10;
circulation_fan3Mode current_circulation_fan3_mode = circulation_fan3_MODE_NATURAL; // Direkt mit Natural starten
int current_circulation_fan3_min_speed = 0; // Standardmäßig 20% Min-Speed
int effective_circulation_fan3_speed = 0;    // Das ist das SPEED_NOW (Ist)
...
void circulation_fan3_init(int pin, int tacho_pin) {
    ...
    circulation_fan3Prefs.begin("circulation_fan3", false);
...
void circulation_fan3_set_speed(int percent) {
    ...
    if(current_circulation_fan3_mode == circulation_fan3_MODE_MANUAL) {
        uint32_t duty = 0;
        if (current_circulation_fan3_speed > 0) {
            duty = map(current_circulation_fan3_speed, 1, 100, 65, 255);
        }
...
void circulation_fan3_set_min_speed(int percent) {
    current_circulation_fan3_min_speed = constrain(percent, 0, 100);
    circulation_fan3_save_state();
}
...
void circulation_fan3_update() {
    ...
    if (current_circulation_fan3_mode == circulation_fan3_MODE_MANUAL) {
        effective_circulation_fan3_speed = current_circulation_fan3_speed;
        uint32_t duty = 0;
    ...
    if (millis() - last_wind_change > 1500) {
        ...
        effective_circulation_fan3_speed = current_circulation_fan3_min_speed + (int)(diff * mix_factor);
