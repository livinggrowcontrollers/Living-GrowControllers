...
class SensorBLEOutsideTile(BoxLayout):
    ...
    def on_touch_down(self, touch):
        ...
        target_tile_id = "ble_temp_outside" 

        # Bestimmen, welches Label getroffen wurde (hit_test)
        if self.lbl_temp.collide_point(*touch.pos):
            target_tile_id = "ble_temp_outside"
        elif self.lbl_hum.collide_point(*touch.pos):
            target_tile_id = "ble_hum_outside"
        elif self.lbl_vpd.collide_point(*touch.pos):
            target_tile_id = "ble_vpd_outside"
        ...
        if hasattr(GLOBAL_STATE, "ggm"):
            try:
                print(f"[BLE_OutsideTile] Klick erkannt! Öffne Graph für: {full_key}")
                ...
                print(f"[BLE_OutsideTile] Fehler beim Öffnen des Fullscreens: {e}")
        else:
            print("[BLE_OutsideTile] Fehler: GLOBAL_STATE.ggm existiert nicht.")
...
# --- FIX: Synchronisiere die ID mit der echten, aktiven Geräte-MAC ---
        if prefix and "_" in prefix:
            # extrahiert die echte MAC und den Kanal aus dem übergebenen Prefix
        ...
        temp_data = outside.get("temperature", {})
        hum_data = outside.get("humidity", {})
        vpd_data = outside.get("vpd", {})
        ...
        if temp_val is not None:
            active_tiles.append("ble_temp_outside")
        if hum_val is not None:
            active_tiles.append("ble_hum_outside")
        if vpd_val is not None:
            active_tiles.append("ble_vpd_outside")
        GLOBAL_STATE.register_tiles(active_tiles, self.device_id, self.channel)

        key_prefix = f"{prefix}_" if prefix else ""

        trend_temp = GLOBAL_STATE.get_trend_icon(f"{key_prefix}ble_temp_outside")
        trend_hum = GLOBAL_STATE.get_trend_icon(f"{key_prefix}ble_hum_outside")
        trend_vpd = GLOBAL_STATE.get_trend_icon(f"{key_prefix}ble_vpd_outside")
