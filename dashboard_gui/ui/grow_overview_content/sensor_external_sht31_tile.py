...
class SensorExternalSHT31Tile(BoxLayout):
    ...
    def on_touch_down(self, touch):
        ...
        # 2. Welches externe Tile-ID (welcher Graph) soll geöffnet werden?
        # Standard-Fallback auf die externe Temperatur
        target_tile_id = "temp_ex" 

        # Bestimmen, welches Label getroffen wurde (hit_test für externe Keys)
        if self.lbl_hum.collide_point(*touch.pos):
            target_tile_id = "hum_ex"
        elif self.lbl_vpd.collide_point(*touch.pos):
            target_tile_id = "vpd_ex"
        elif self.lbl_temp.collide_point(*touch.pos):
            target_tile_id = "temp_ex"

        # 3. Den magischen Full-Key zusammenbauen
        # Nutzt wieder device_id und channel der Kachel, hängt aber das externe Codewort an
        ...
        if hasattr(GLOBAL_STATE, "ggm"):
            try:
                print(f"[External-Tile] Klick erkannt! Öffne Graph für: {full_key}")
                ...
                print(f"[External-Tile] Fehler beim Öffnen des Fullscreens: {e}")
        else:
            print("[External-Tile] Fehler: GLOBAL_STATE.ggm existiert nicht.")
    ...
    def update_values(self, data, prefix=""):
        ...
        if temp_val is not None:
            active_tiles.append("temp_ex")
        if hum_val is not None:
            active_tiles.append("hum_ex")
        if vpd_val is not None:
            active_tiles.append("vpd_ex")
